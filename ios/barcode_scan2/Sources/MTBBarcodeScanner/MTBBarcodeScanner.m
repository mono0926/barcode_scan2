...
- (void)stopScanning {
    ...
    AVCaptureVideoPreviewLayer *layer = self.capturePreviewLayer;
    ...
    dispatch_async(self.privateSessionQueue, ^{
        ...
        // Disconnect preview layer AFTER session has fully stopped to avoid
        // blocking on iOS 26+ where AVFoundation waits for camera pipeline sync
        dispatch_async(dispatch_get_main_queue(), ^{
            layer.session = nil;
        });
